...
public class SUILayoutCreator
{
	...
	private static SUIProcess CreateSprite(SDFTreeNode data ) {
		...
		if (data["fitWidth"] != string.Empty ) {
			sUISprite.autoscaleFitWidth = SUILayoutConv.GetBool(data["fitWidth"]);
		}
		return sUISprite;
	}
	...
	private static SUIProcess CreateButton(SDFTreeNode data ) {
		...
		if (data["scale"] != string.Empty ) {
			sUIButton.scale = SUILayoutConv.GetVector2(data["scale"]);
		}
		if (data["priority"] != string.Empty ) {
		...
		if (data["text"] != string.Empty ) {
			sUIButton.text = SUILayoutConv.FormatNewlines(Singleton<Localizer>.instance.Parse(data["text"]));
		}
		...
		if (data["fitWidth"] != string.Empty ) {
			sUIButton.autoscaleFitWidth = SUILayoutConv.GetBool(data["fitWidth"]);
		}
		return sUIButton;
	}
	...
	private static SUIProcess CreateLabel(SDFTreeNode data ) {
		...
		if (data["text"] != string.Empty ) {
			sUILabel.text = SUILayoutConv.FormatNewlines(Singleton<Localizer>.instance.Parse(data["text"]));
		}
		if (data["position"] != string.Empty ) {
			sUILabel.position = SUILayoutConv.GetVector2(data["position"]);
		}
		if (data["scale"] != string.Empty ) {
			sUILabel.scale = SUILayoutConv.GetVector2(data["scale"]);
		}
		if (data["priority"] != string.Empty ) {
