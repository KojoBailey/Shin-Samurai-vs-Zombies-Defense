...
public class StoreAvailability
{
	...
	private static void GetBoosterPack(List<StoreData.Item> items)
	{
		if (!(Singleton<PlayModesManager>.instance.selectedMode != "classic"))
		{
			...
			item.title = SUILayoutConv.FormatNewlines(Singleton<Localizer>.instance.Get("normal_store_link_title"));
			items.Add(item);
		}
	}
	...
	public static void GetVIP(List<StoreData.Item> items)
	{
		...
		item.title = SUILayoutConv.FormatNewlines(Singleton<Localizer>.instance.Get((num <= 0) ? "vip_btn_title" : "vip_btn_title_with_plan"));
		items.Add(item);
	}
}
