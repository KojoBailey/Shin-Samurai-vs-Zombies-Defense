...
public class Profile : Singleton<Profile> {
	...
	public bool wasBasicGameBeaten {
		get {
			// Checks if the final wave for a mode has been beaten at least once.
			return GetWaveLevel(maxBaseWave) > 1;
		}
	}
	public bool tutorialIsComplete {
		get {
			// Checks if the first 3 waves (tutorials) have been beaten at least once.
	...
	public string playModeSubSection {
		// Classic Mode or Zombies Rising
		get {
	...
	public string swordID {
		get {
			...
			if (value == string.Empty) {
				return Singleton<HeroesDatabase>.instance.GetAttribute(Singleton<Profile>.instance.heroID, "meleeWeapon");
	...
	public string bowID {
		get {
			...
			if (value == string.Empty) {
				string weaponID = Singleton<HeroesDatabase>.instance.GetAttribute(Singleton<Profile>.instance.heroID, "rangedWeapon");
				if (weaponID == string.Empty) return "Bow";
				return weaponID;
	...
	/* Heroes */
	public int GetHeroLevel(string heroID_in) {
		return Mathf.Max(1, mSavedData.GetValueInt("heroLevel", heroID_in));
	}
	public void SetHeroLevel(string heroID_in, int val) {
		mSavedData.SetValueInt("heroLevel", Mathf.Max(0, val), heroID_in);
	}
