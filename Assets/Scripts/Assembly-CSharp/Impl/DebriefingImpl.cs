...
public class DebriefingImpl : SceneBehaviour
{
		...
		if (Singleton<PlayStatistics>.instance.stats.lastWaveWon)
		{
			...
			if (!Singleton<Profile>.instance.wasBasicGameBeaten && !Singleton<Profile>.instance.neverShowRateMeAlertAgain)
			{
				...
				if (lastWavePlayed == 5 || lastWavePlayed == 10 || lastWavePlayed == 20 || lastWavePlayed == 30 || lastWavePlayed == 40)
				{
					...
					{
						m_rateDialog = new TwoOptionDialog(SUILayoutConv.FormatNewlines(Singleton<Localizer>.instance.Get("like_our_game_android_amazon")), false, SUILayoutConv.FormatNewlines(Singleton<Localizer>.instance.Get("rate_it_android")), delegate
						{
					...
					{
						m_rateDialog = new TwoOptionDialog(SUILayoutConv.FormatNewlines(Singleton<Localizer>.instance.Get("like_our_game_android")), false, SUILayoutConv.FormatNewlines(Singleton<Localizer>.instance.Get("rate_it_android")), delegate
						{
							OnClickRateDialog(true);
						}, Singleton<Localizer>.instance.Get("cancel_rating_android"), delegate
						{
	...
	private IEnumerator AwardGameCurrencyAfterRating()
	{
		...
		m_rateAwardDialog = new YesNoDialog(SUILayoutConv.FormatNewlines(Singleton<Localizer>.instance.Get("rating_award")), false, delegate
		{
		}, null);
